...
namespace AppEscritorio.General
{
    ...
    public partial class FrmPermisos : Form
    {
        ...
        private void FrmPermisos_Load(object sender, EventArgs e)
        {
            ...
            ListadoUsuarios = (from x in objCapaLogica.ConsultarUsuarios()
                              select new UsuAux
                              {
                                  IdUsuario = x.IdUsuario,
                                  Usuario = x.Usuario,
        ...
        {
            if (TxtUsuario.Text.Length > 0 || (TxtUsuario.Text.Length > 0 && (e.KeyChar == (char)Keys.Back)))
            {
                DgUsuarios.DataSource = ListadoUsuarios.Where(x => (x.Usuario + x.NomUsu).Trim().ToUpper().Contains(TxtUsuario.Text.ToUpper().Trim())).ToList();

            }
            else
            {
                DgUsuarios.DataSource = ListadoUsuarios.ToList();
            }
        }
    }
}
