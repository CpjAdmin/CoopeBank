    ...
    public partial class FrmNominaCGP : Form
    {
        ...
        private void ConsultarTipoNomina()
        {
            try
            {
                ...
                this.cmbTipoNomina.SelectedIndex = 2;

                MessageBox.Show(cmbTipoNomina.SelectedIndex.ToString());
            }
            catch (Exception ex)
            {
                    ...
                    if (saveFile.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        foreach (DataGridViewRow row in dtDatos.Rows)
                        {
                            if (Convert.ToBoolean(row.Cells[9].Value))
                            {
                                ...
                                if (dato.CuentaClienteDestino.Length == 22)
                                {
                        ...
                        if (ListadoCuentasIBANPIN.Count > 0) {
                            ToolSistema.PrintXMLPIN(this.saveFile.FileName, ListadoCuentasIBANPIN,"PagoNomina");
                        }
        ...
        {
            bool rs = await ToolSistema.IsPINEntity("CR71010000099801000159");
